...
const Game: React.FC = () => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    if (!gameStarted || gameOver || gamePaused || !gameAreaRef.current) return;
    ...
    setPlayerPos({
      x: Math.max(0, Math.min(GAME_WIDTH - PLAYER_SIZE, mouseX - PLAYER_SIZE / 2)),
      y: Math.max(0, Math.min(GAME_HEIGHT - PLAYER_SIZE, mouseY - PLAYER_SIZE / 2))
    });
  ...
  const gameLoop = useCallback((currentTime: number) => {
    ...
    // Spawn bullets
    if (currentTime - lastBulletTime.current > difficultySettings.bulletRate) {
    ...
    if (currentTime - lastMeteorTime.current > difficultySettings.meteorSpawnRate) {
      ...
      <div className="game__container">
        <div className="game__header">
          ...
          <p className="game__subtitle">
            Survive the meteor storm! Move your mouse to navigate, auto-fire destroys meteors.
          </p>
        </div>
          ...
          {gameOver && (
            <div className="game__game-over">
              ...
              <div className="game__final-score">
                ...
                <p>Level Reached: <strong>{difficulty}</strong></p>
